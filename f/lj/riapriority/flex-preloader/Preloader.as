package 
{
	...
	{
		private static const PROGRESS_WHEN_LOADED:Number = 0.5;
		...
		private var assets:PreloaderAssets;
		...
		{
			assets = new PreloaderAssets();
			assets.x = 0;
			assets.y = 0;
			assets.getChildByName("error").visible = false;
			addChild(assets);
			progress = 0;
		...
		{
			assets.getChildByName("error").visible = true;
		}
		...
		protected function set progress(value:Number):void
		{
			...
			var frontTf:TextField = (assets.getChildByName("front") as TextField);
			var backTf:TextField = (assets.getChildByName("back") as TextField);
			frontTf.text = 
			backTf.text = percent.toString();
		...
		{
			// Не делаем ничего, нужно дождаться установки applicationInitProgress = 1
		}
